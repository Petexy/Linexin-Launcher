...
- **The launcher stays on top of everything while it is open.** Nothing kept it there: the window asked for no stacking hint at all, and a fullscreen window only sits above the rest while it is the *active* one — so the instant anything else took focus (an app mapping its own window, a global shortcut, a click on a second screen) the launcher dropped to the ordinary window layer and the panel and other windows drew straight over it, leaving it stranded behind them. It now requests stays-on-top, and re-takes activation whenever it loses it, which is what puts it back above other windows on Wayland where no keep-above request exists. Closing and launching an app are exempt, so the app you just started still gets the focus.
